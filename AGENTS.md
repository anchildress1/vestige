...
24. **Open PRs only when the user says so.** Cutting a branch, committing, and pushing the branch are all authorized by the implementation work itself. Running `gh pr create` (or any equivalent PR-open action) requires an explicit user instruction in the current turn. Default: push the branch, surface it in chat, wait. The user decides when a PR opens, what gets bundled into it, and what stays as a draft on the side. This rule kills the "another PR??" failure mode where small, related changes get fragmented into multiple review threads the user did not authorize.
