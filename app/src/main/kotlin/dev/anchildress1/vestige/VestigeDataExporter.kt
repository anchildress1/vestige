...
internal class VestigeDataExporter(
    ...
    fun writeTo(out: OutputStream) {
        ZipOutputStream(out).use { zip ->
            zip.putTextEntry(SNAPSHOT_ENTRY, buildSnapshot().toString(JSON_INDENT))
            markdownStore.listAll().forEach { file ->
                zip.putFileEntry(file)
            }
        }
    }

    private fun buildSnapshot(): JSONObject = JSONObject()
        ...
        .put("markdown_files", markdownFilesJson())
    ...
    private fun markdownFilesJson(): JSONArray = markdownStore.listAll()
        .map { it.name }
        .sorted()
        .toJsonArray()
