...
- Given the user records a new entry, when the model returns transcription + follow-up, then that exchange is one complete entry. Starting another recording begins a new entry — the prior turn is not threaded into the new prompt. Per `adrs/ADR-005-stt-b-scope-and-v1-single-turn.md` (amends `adrs/ADR-002-multi-lens-extraction-pattern.md` §"Multi-turn behavior"), the prompt-stuffing multi-turn pattern (fresh `engine.createConversation()` per turn + prior turns crammed into the system prompt) was tested across three rounds on E4B CPU and produced retention=0.0; the LiteRT-LM SDK's stateful Conversation path was not measured. v1 scopes to single-turn-per-capture for simplicity rather than ship a session UX whose pattern is unverified.
...
- ~~Does multi-turn conversation work on E4B with our prompt patterns?~~ **Partially resolved 2026-05-09.** Three STT-B rounds on E4B CPU (S24 Ultra) produced retention=0.0 under the prompt-stuffing pattern (fresh `engine.createConversation()` per turn + history crammed into the system prompt) — even after a Round 3 rewrite added an explicit instruction telling Gemma to reference prior turns. The LiteRT-LM SDK's stateful Conversation path (one persistent Conversation handle across multiple sendMessage calls) was **not** measured; whether E4B can sustain multi-turn through the SDK's intended path is open. v1 scopes to single-turn-per-capture for simplicity rather than ship a multi-turn UX whose pattern is unverified. See `adrs/ADR-005-stt-b-scope-and-v1-single-turn.md` (amends `adrs/ADR-002-multi-lens-extraction-pattern.md` §"Multi-turn behavior") for the round-by-round record + the test surface a future revival should exercise.
...
| **STT-B** | Multi-turn conversation reliability on E4B | Phase 2, capture loop | Model loses session context across 3+ turns | **PARTIALLY MEASURED 2026-05-09 — v1 scopes to single-turn.** See verdict note below the table. |
...
**STT-B verdict note (2026-05-09):** the original STT-B contract — "model loses session context across 3+ turns" — was tested across three rounds on E4B CPU (S24 Ultra). All three rounds exercised one specific multi-turn pattern: per turn, `LiteRtLmEngine.sendMessageContents` opens a fresh `engine.createConversation()` and stuffs prior turns' transcribed text into the system prompt as a `## RECENT TURNS` block. **Result under that pattern: retention=0.0/3 sessions.** The LiteRT-LM SDK's stateful `Conversation` path (one persistent handle, multiple `sendMessage` calls so the SDK's native KV cache + dialogue context carry across turns) was **not** measured; whether E4B sustains multi-turn through the SDK's intended path is open. v1 ships single-turn-per-capture for simplicity, not because the multi-turn capability was conclusively absent. See `adrs/ADR-005-stt-b-scope-and-v1-single-turn.md` (which amends `adrs/ADR-002-multi-lens-extraction-pattern.md` §"Multi-turn behavior") for the round-by-round record + the test surface a future revival should exercise.
...
2. ~~Multi-turn session state preservation.~~ **🛑 STT-B PARTIALLY MEASURED 2026-05-09 — v1 scopes to single-turn.** Three rounds on E4B CPU produced retention=0.0 under the prompt-stuffing pattern (fresh `createConversation()` per turn + history in system prompt); the SDK's stateful Conversation path was not measured. v1 scopes to single-turn-per-capture for simplicity. The capture loop: each new tap of record begins a fresh session and the model never sees prior turns. See `adrs/ADR-005-stt-b-scope-and-v1-single-turn.md` (amends `adrs/ADR-002-multi-lens-extraction-pattern.md` §"Multi-turn behavior").
...
4. Convergence resolver (consensus / candidate / ambiguous per field). **🛑 STT-C — tag extraction consistency.** Run 10+ varied test dumps through the resolver; verify stable tag emission. If unstable, tighten prompts.
