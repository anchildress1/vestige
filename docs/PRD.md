...
| **STT-B** ~~| Multi-turn conversation reliability on E4B | Phase 2, capture loop | Model loses session context across 3+ turns~~ **PARTIALLY MEASURED 2026-05-09 — v1 scopes to single-turn.** Three rounds on E4B CPU (S24 Ultra) produced retention=0.0 under the prompt-stuffing pattern (fresh `createConversation()` per turn + history in system prompt). The SDK's stateful Conversation path was not measured. v1 ships single-turn-per-capture for simplicity, not because E4B's multi-turn capability was conclusively absent. See `adrs/ADR-005-stt-b-scope-and-v1-single-turn.md` (amends `adrs/ADR-002-multi-lens-extraction-pattern.md` §"Multi-turn behavior") for the round-by-round record + the test surface a future revival should exercise. |
