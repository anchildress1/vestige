...
- All Phase 4 story references to `poc/design-review.md` and `poc/screenshots/{capture,recording,roast,reading,persona,local-model,destruction,wiped,typed,error}.png` (file is deleted; screenshots are deleted). New canonical POC: the flat `poc/*-final.png` reference renders — `poc/roast-final.png` (hero), `poc/system-final.png` (scorecard), `poc/capture-idle-populated-final.png` (capture-still), `poc/capture-recording-final.png` (capture-running), `poc/patterns-final.png`, `poc/pattern-detail-final.png`, `poc/vocab-drift-final.png`.
