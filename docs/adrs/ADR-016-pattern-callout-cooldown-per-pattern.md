...
| ADR-003 §Cooldown "Mechanics" bullet 1: "Persist `lastCalloutEntryId: Long?` and `lastCalloutTimestamp: Long?` as **singleton** settings (one row per app, not per pattern)." | Singleton entity | One row per `patternId`. `CalloutCooldownEntity` gains an indexed `patternId: String` column. The singleton accessor (`snapshot()`) is replaced by `snapshotFor(patternId)`. |
| ADR-003 §Cooldown "Why global, not per-pattern" | Justified rejection of per-pattern cooldown | Rejection no longer holds — the two-callouts-on-the-same-entry concern is preserved by keeping the existing single-selection rule (one pattern per entry); the cooldown's job is "don't repeat *this* observation," not "shut up briefly." |
...
- **`recordFired` / `confirmReservedCallout` / `releaseReservedCallout` semantics** — same lifecycle, same race-safety; just keyed by `patternId` instead of singleton.
...
When the implementing commits land, the following surfaces change:

- `core-storage/.../CalloutCooldownEntity.kt` — `@Index var patternId: String` column.
- `core-storage/.../CalloutCooldownStore.kt` — every method that took `entryId` now also takes `patternId`. `snapshot()` → `snapshotFor(patternId)`. New `decrementAllActive()` for the per-entry sweep. `clearStalePendingReservation()` sweeps every row.
- `app/.../patterns/PatternDetectionOrchestrator.kt` — `onEntryCommitted` flips order: choose matching pattern first (pure), then `tryReserveCallout(entryId, matchedPatternId)`. `settleReservedCallout(entry, patternId, fired)` takes the patternId.
- `app/.../save/BackgroundExtractionSaveFlow.kt` — the consumer wiring passes `patternId` through to `settleReservedCallout`.
- ObjectBox schema bump — model.json picks up the new `patternId` column on `CalloutCooldownEntity`.
- Tests cover: independent per-pattern counters, multiple patterns firing in succession, same-pattern suppression for 3 entries, mixed pattern firing during another pattern's cooldown, `clearStalePendingReservation` sweep, `decrementAllActive` idempotence at zero.
