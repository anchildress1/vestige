...
| ~~`RetrievalRepo`~~ | — | **Removed 2026-05-24** — cut with the tone-word embedding axis change; content retrieval is incoherent against a feeling vector. See §"Embedding Strategy" addendum (2026-05-24). |
...
**Query side (removed 2026-05-24):** `RetrievalRepo.query` embedded the raw user query as-is against distilled semantic content. This whole content-retrieval path was cut when the embedding axis moved to the tone word — see the §"Embedding Strategy" addendum below.
...
- **Consequence for retrieval — RetrievalRepo CUT (2026-05-24).** The vector is now the *feeling* axis, so a content query against tone-word entry vectors is incoherent. `RetrievalRepo`, `AppContainer.retrieveHistory`, `RetrievalRepoTest`, and the STT-E gate (`SttEEmbeddingComparisonTest` + `SttEManifest`) were deleted (`backlog.md` → `embedding-retrieval-surface`, resolved: cut). `EntryEntity.vector` now serves vocab clustering only. Reviving content retrieval later needs its *own* second content vector — not this one.
- **Thresholds unchanged — verified on-device (2026-05-24).** The fear that `0.30` was wrong for tone-word distances didn't hold: with 18 toned entries the clustering produced `clusterSizes=[6,4,2,1,1,1,1,1,1]` at the unchanged `maxCosine=0.30` (repeated/synonymous tones land at ~0–0.21; distinct tones at 0.31+). `Drained Vocab Frequency` is now a live ACTIVE pattern. `DEFAULT_MAX_COSINE_DISTANCE` / `VOCAB_THRESHOLD` / `MIN_SUPPORTING_ENTRIES` all stay as-is; `detectVocab` keeps logging candidate count + cluster sizes + nearest-neighbor distances as a calibration aid.
