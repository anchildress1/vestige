...
| ~~`gpu-model-artifact`~~ | — | — | **Resolved 2026-05-10 — wrong premise.** The artifact was always GPU-capable; the manifest missed `libOpenCL.so` and `libvndksupport.so` `<uses-native-library>` declarations LiteRT-LM needs on Android 12+. Fix landed in `AndroidManifest.xml`. GPU now runs ~8–13 s per lens call (vs ~35–55 s CPU). See Story 2.7 GPU re-run notes for the latency record and the parse-failure observation that surfaced on GPU but not CPU. | n/a — closed |
