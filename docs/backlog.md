...
### `archetype-template-labeling` (UI yank done via Story 4.16; redesign is v1.5)
...
fix-path: add diagnostic logging (vectored count / cluster sizes / max merge distance) → re-seed EXTRACT=1
         → measure → calibrate cut / embedded-text / threshold at root → prove the cluster mints on-device.
