...
| `backfill-on-artifact-complete` | Phase 4 | onboarding | **Logged 2026-05-12** from PR #22 Codex review. Currently `launchVectorBackfillIfReady` triggers only on cold start + `saveAndExtract`. If embedding artifacts finish downloading mid-session and the user doesn't save, null-vector rows stay null until next save / restart. Onboarding (Phase 4 — Story 4.x) wires the model-download UI; the natural hook is to call `appContainer.launchVectorBackfillIfReady()` on transition to `Complete`. AtomicBoolean CAS guard makes the call idempotent and safe at any time. | Phase 4 model-download UX story lands |
