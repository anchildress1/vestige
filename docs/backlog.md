...
| `mtp-speculative-decoding` | v1.5 | inference | GPU backend now active; MTP needs `ExperimentalFlags` before `engine.initialize()`; API not yet stable. Drafter (`tf_lite_mtp_drafter`) is already bundled in the E4B model file — no separate download. Expected 1.7–2.2x decode speedup. [Ref](https://blog.google/innovation-and-ai/technology/developers-tools/multi-token-prediction-gemma-4/) | MTP exits `ExperimentalFlags` in a LiteRT-LM release |
