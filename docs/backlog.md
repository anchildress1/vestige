...
| `gpu-model-artifact` | stt-C | inference | E4B `.litertlm` on the device is CPU-compiled per ADR-001 §Q3; `Backend.GPU()` fails native init (`llm_litert_compiled_model_executor.cc:1928`). CPU latency (~30–55 s per lens call, ~127–161 s per 3-lens entry) makes repeat-run sweeps like STT-C impractical (~2.5 hr for 162 calls), so STT-C measurement defers. STT-D measurement (one pass per entry, 6 entries) ran cleanly on CPU and passed — see Story 2.7 | a GPU-compiled E4B `.litertlm` variant exists on Hugging Face or via local conversion AND the LiteRT-LM SDK version pinned by `gradle/libs.versions.toml` loads it. Then run `SttCTagStabilityTest -PinferenceBackend=gpu` end-to-end and record the stability rate in Story 2.9 |
