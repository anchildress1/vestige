...
| `mtp-speculative-decoding` | v1.5 | inference | GPU backend now active; MTP needs `ExperimentalFlags` before `engine.initialize()` + drafter model download; API not yet stable. Expected 1.7–2.2x decode speedup. [Ref](https://blog.google/innovation-and-ai/technology/developers-tools/multi-token-prediction-gemma-4/) | MTP exits `ExperimentalFlags` in a LiteRT-LM release |
