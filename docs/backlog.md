...
| `multi-chunk-foreground` | stt-A | inference | `AudioCapture` already emits `isFinal=false` intermediate chunks for >30 s captures (Story 1.4), but `ForegroundInference.runForegroundCall` rejects them — orchestration (stripped-down transcription-only call per intermediate chunk + concatenated transcript-so-far injected on the final chunk per ADR-002 §"For >30s captures") is unwritten. Single-narrative recordings cover the demo's 90 s pitch + 5 min walkthrough; long-dump pathway is not on the critical path | user gives a >30 s entry attempt and it fails / sample-data-scenarios §STT-A chunk-boundary script gets exercised end-to-end (pre-split files at `~/Documents/stt-a1.wav` + `stt-a2.wav` from 2026-05-09 are sitting on disk for that future run) |
| `smart-turn-boundaries` | stt-B | inference | `CaptureSession` is append-only — every record-then-respond cycle adds a USER+MODEL pair to the same transcript and `## RECENT TURNS` keeps showing the prior 4 forever. There's no signal for "this new dump is unrelated; reset the session" so unrelated captures inside one CaptureSession contaminate each other's context. Today's CaptureSession lifecycle conflates "one entry's worth of conversation" with "the session instance" — fine for the demo's bounded test scripts; wrong as soon as a real user holds the app open across topics. Candidate triggers: idle-time gap, save-flow boundary (Story 2.12 fires `acknowledgeResponse` → next capture starts fresh), explicit UI reset, model-side topic-shift heuristic | STT-B verdict locks in (round 3 in flight 2026-05-09); if multi-turn survives, design the reset rule before Phase 4 history UI lands. If multi-turn dies, this entry collapses — single-turn captures don't accumulate session state |
