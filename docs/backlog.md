...
| ~~`smart-turn-boundaries`~~ | — | — | **Collapsed 2026-05-09** when STT-B failed and v1 went single-turn-per-capture. With each tap of record producing a fresh `CaptureSession` and no prior-turn context threaded into the prompt, there is no session boundary to be smart about. | n/a — entry retained in the table only as a tombstone explaining why the original concern dissolved |
