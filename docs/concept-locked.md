...
## Personas (three settable; default Witness; per-session override allowed)
...
**Two-tier processing:**
- *Foreground:* fast pass returns transcription + follow-up question only. Conversation flows at human pace.
...
- **Transcription handling:** Gemma 4 returns both the transcription and the follow-up question in a single structured response. Transcription is shown in the conversation transcript (visually secondary to the model's response — see design-guidelines.md). Transcription is saved as `entry_text` (the substrate of the entry). Audio bytes are discarded after the model call.
