...
- 30s cap pre-warn at 28s elapsed: single-fire system tone on the notification stream (not a bundled chime). Minimum-viable signal so the user knows the cap is firing — no cap-moment cue, the brief submitting spinner carries that.
