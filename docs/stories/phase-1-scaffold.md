...
**Done when:**
- [x] A test harness accepts a normalized audio buffer from `AudioCapture` (Story 1.4) and produces a transcription via Gemma 4 E4B. _(Harness ships as `SttAProbe` in `:core-inference` instrumentation test sources at `core-inference/src/androidTest/.../inference/SttAProbe.kt`; runtime WAV encoding helper remains `WavWriter` in `:core-inference`.)_
...
**Done when:**
- [x] `:core-inference` test source set contains a `ConvergenceResolverTest` class with stub test cases covering each convergence outcome from `concept-locked.md` §"Convergence rules": canonical (≥2 of 3 agree), candidate (only Inferential populates), ambiguous (lenses disagree), consensus-with-conflict (Skeptical flags conflict).
...
- [x] At least one happy-path test case (all three lenses return identical structured output → consensus) is *fully written* as a documentation example, even though the underlying resolver isn't implemented yet.
