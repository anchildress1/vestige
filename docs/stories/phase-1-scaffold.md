...
**Done when:**
- [ ] A test harness in `:core-inference` accepts a normalized audio buffer from `AudioCapture` (Story 1.4) and produces a transcription via Gemma 4 E4B.
- [ ] At least 5 sample utterances (varied length within 30s, varied content per `sample-data-scenarios.md`) round-trip through the harness on the reference S24 Ultra and produce coherent transcriptions matching what was spoken (≥90% word-accurate by visual inspection — this is not a benchmark, it's a smoke test).
- [ ] The exact LiteRT-LM Android handoff is documented inline in ADR-001 Q4: which of `Content.AudioBytes(audioBytes)` vs `Content.AudioFile(path)` is the working path, and the exact byte packing or file format used.
- [ ] If `AudioFile` is the working path, temp-file lifecycle is correct: created → handed to model → deleted within the same call. No persisted audio.
- [ ] Latency of one 30-second clip end-to-end (capture stop → transcription back) recorded to ADR-001 §Latency notes.
...
- [ ] Audio is not written. Per `AGENTS.md` guardrail 11, only transcription text persists.
