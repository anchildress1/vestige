...
- [ ] At least 5 sample utterances (varied length within 30s, varied content per `sample-data-scenarios.md`) round-trip through the harness on the reference S24 Ultra and produce coherent transcriptions matching what was spoken (≥90% word-accurate by visual inspection — this is not a benchmark, it's a smoke test). _(Manual — user records utterances, adb-pushes, runs `./gradlew :app:connectedDebugAndroidTest -P...`. Box stays unchecked until the user reports green.)_
