...
**Done when:**
- [x] `:core-inference` test source set contains a `ConvergenceResolverTest` class with stub test cases covering each convergence outcome from `concept-locked.md` §"Convergence rules": consensus (≥2 of 3 agree), candidate (only Inferential populates), ambiguous (lenses disagree), consensus-with-conflict (Skeptical flags conflict).
