...
- [x] Per-entry operational fields per ADR-001 Q3 (`extraction_status`, `attempt_count`, `last_error`) are updated as the worker progresses through the three calls. _(The worker emits transitions through `ExtractionStatusListener.onUpdate(status, entryAttemptCount, lastError)`: one `RUNNING` at start, one `RUNNING` per lens-level retry with the same entry retry count + latest failure, and exactly one terminal `COMPLETED` or `FAILED`. `last_error` is cleared on `COMPLETED` and populated only on `FAILED`; the listener is the seam the persistence layer uses to mirror onto the `EntryEntity` row — `:core-inference` does not depend on `:core-storage`.)_
