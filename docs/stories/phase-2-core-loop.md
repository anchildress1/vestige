...
- [/] Measure foreground call latency on the reference S24 Ultra before and after. Record delta in logcat under `VestigeLiteRtLm`. A >1.5x decode speedup is the acceptance threshold; if below 1.5x, document why and leave MTP enabled regardless (correctness is primary). _(**Deferred 2026-05-17 — non-gating, not measured.** Per the story's own rule MTP stays enabled regardless of the ratio, so the A/B decides nothing; it is not demo-visible (not in the 90s pitch / 5min walkthrough); correctness is pinned by `LiteRtLmEngineTest."initialize turns on MTP speculative decoding before engine construction"`. Measuring requires two invasive builds (no runtime MTP toggle — `LiteRtLmEngine.kt:58` is unconditional). Demo-gate (AGENTS.md): doesn't improve pitch/walkthrough → `backlog.md` §`mtp-latency-ab`. The number is a v1.5 talking-point nicety, not a v1 blocker.)_
- [/] Measure background lens call latency (one lens call, no convergence) before and after. Record delta. _(**Deferred 2026-05-17 — non-gating, not measured.** Same rationale as the foreground bullet above; `backlog.md` §`mtp-latency-ab`.)_
...
- [x] `CaptureViewModel.Reviewing` state carries a `followUpText: String` field that appends tokens as they arrive. The capture screen renders the growing string in real time. _(`ReviewState.followUp` grows via `showReviewing` on each `FollowUpDelta`; `Transcription` surfaces first, `Terminal(Success)` replaces the streamed approximation with the authoritative parsed value + saves. `Reviewing.streaming` is `true` until the terminal event; `CaptureScreen` renders `ReviewState.followUp` verbatim and withholds the Done button while streaming so the entry can't be acknowledged before it is persisted.)_
...
- [ ] On-device: subjective latency improvement is visible on the reference S24 Ultra. Time-to-first-token logged under `VestigeForegroundInference`. _(**Manual-check stop — on-device subjective improvement. Not self-ticked.** The loggable half is shipped: `emitEnvelope` emits `Log.d("VestigeForegroundInference", "<label> persona=… ttft=<ms>ms")` on the first model stream chunk (not the first scanner-surfaced event — the scanner withholds until a tag closes, which would inflate TTFT by the whole transcription block). The S24 Ultra subjective check awaits user outcome.)_
