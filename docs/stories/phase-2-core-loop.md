...
- [ ] All existing unit + integration tests pass. _(Gated by the `lefthook` pre-push `make build+test` run — not double-run manually per the test-harness discipline. JVM unit coverage added: `LiteRtLmEngineTest."initialize turns on MTP speculative decoding before engine construction"` asserts the flag flips before the native boundary. Tick once the pre-push gate is green.)_
