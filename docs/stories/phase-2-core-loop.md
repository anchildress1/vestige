...
- [x] `ForegroundInference.runForegroundCall()` switches from `sendMessageContents()` (blocking) to `streamMessageContents()`. The return type changes from a single `ForegroundResult` to a `Flow<ForegroundToken>` or equivalent streaming shape — design this surface so `CaptureViewModel` can update its `Reviewing` state incrementally. _(Both `runForegroundCall` (voice) and `runForegroundTextCall` (typed) now return `Flow<ForegroundStreamEvent>` — a `Transcription` / `FollowUpDelta` / `Terminal` sealed surface. Shared private `emitEnvelope` extension keeps scan + parse logic single-sourced across both paths. No backward-compat shim — old blocking signature deleted per AGENTS.md.)_
...
**As** the AI implementor, **I need** retrieval history to enrich follow-up generation and background extraction **without** blocking entry creation, **so that** the user sees the transcript-backed entry immediately while detached work still gets prior-entry context.

**Done when:**
- [x] Voice and typed capture both save/open first; retrieval runs only on detached follow-up + detached extraction. _(`CaptureViewModel` no longer calls `retrieveHistory` before `saveAndExtract(...)` / `openEntry(...)`. `launchFollowUp(...)` retrieves history after the handoff, and `BackgroundExtractionSaveFlow` performs retrieval itself when the caller seeded none.)_
- [x] The retrieved history is threaded into detached `runForegroundTextCall()` follow-up generation and into `BackgroundExtractionWorker`, not into the initial foreground save/open path. _(`ForegroundInference.runForegroundCall(audio, persona)` remains history-free; `runForegroundTextCall(text, persona, retrievedHistory)` still accepts history for the detached follow-up path. `BackgroundExtractionSaveFlow.resolveRetrievedHistory(...)` loads history before `worker.extract(...)`.)_
- [x] `AppContainer.retrieveHistory(query)` is the shared seam for detached follow-up generation and detached extraction. _(`AppContainer` passes `::retrieveHistory` into `BackgroundExtractionSaveFlow`; `CaptureViewModel.retrieveHistorySafely(...)` uses the same seam for follow-up generation.)_
...
- [x] Unit tests prove retrieval is no longer on the critical path and still reaches detached work. _(`voice path opens entry before lookup resolves and threads history only to call-2`, `submitTyped threads looked-up history only to call-2`, plus `BackgroundExtractionSaveFlowTest` coverage for detached retrieval, caller-seeded history, and degraded lookup.)_

**Notes / risks:** Corrected 2026-05-19 for latency. Retrieval is no longer allowed to sit between transcription and entry-open on either capture path. Gemma execution is still serialized by `LiteRtLmEngine`, so detached follow-up / extraction wait for foreground decode to release the GPU; this change removes query-embedding / vector-search latency from the critical path, not model latency itself.

---

### Story 2.19 — Concurrent inference: eliminate foreground-blocks-on-background mutex contention — **WITHDRAWN 2026-05-17 (SDK-impossible on 0.11.0 — STT-F; foreground does serialize behind background)**
...
- [x] No new entries logged to `backlog.md` from Phase 2 work that change the v1 contract beyond what an STT fallback already required. _(The earlier `parallel-lens-execution-via-clone` backlog entry came from the mistaken ADR-009 supersede; ADR-009 is deleted and that entry is retired — concurrent multi-context was implemented under Stories 2.6.6 + 2.19 (operator-adopted 2026-05-17), not a backlog SDK-gap item. The v1 contract is now concurrent multi-context per ADR-008 §Addendum (2026-05-17); the only open item is the on-device RAM/wall-clock manual-check measurement.)_
