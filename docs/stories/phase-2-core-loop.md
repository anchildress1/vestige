...
- [x] State machine implemented per ADR-004 §"State Machine": `NORMAL → PROMOTING → FOREGROUND → KEEP_ALIVE → DEMOTING → NORMAL`. _(Pure-Kotlin `BackgroundExtractionLifecycleStateMachine` drives the table; the bus translates `Entry.extraction_status` flips into in-flight count deltas; persistence calls `bus.report(entryId, status)` to drive transitions.)_
