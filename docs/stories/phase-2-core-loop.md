...
- [/] Agent-emitted template labels work for all six archetypes on representative sample transcripts. _(**Won't pass as designed — structural defect found 2026-05-17.** `TemplateLabeler` consumes only CONSENSUS fields; archetype triggers are inferences only the Inferential lens emits → resolves CANDIDATE → discarded → every realistic entry is `AUDIT`. Non-AUDIT only fires when the user speaks the exact internal trigger vocabulary, which is the keyword-stuffed fixture, not a real test. Code stays in place through Phase 2/3; the UI yank is queued in Phase 4 §Story 4.16 and has not landed on this branch yet. Root cause + redesign condition: `backlog.md` §`archetype-template-labeling`. Story 2.10 below carries the full trace.)_
