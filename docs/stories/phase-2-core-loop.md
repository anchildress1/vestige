...
**Future revival pointer:** if multi-turn comes back into scope post-v1, the test surface to exercise is `engine.createConversation()` once per session, then multiple `conversation.sendMessage(Contents.of(...))` calls within that single handle — relying on the SDK's native KV cache + dialogue management instead of injecting prior turns into the prompt. The deleted harness's manifest grammar + retention-scorer logic + the Round 1 false-positive lesson on substring-anchor design all live in ADR-002 §"Multi-turn behavior" as the reference for that future work.
