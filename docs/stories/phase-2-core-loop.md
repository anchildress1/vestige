...
- [/] Synthetic coverage only: `TemplateLabelerTest` hand-feeds canonicalized field maps for all six labels, plus precedence and boundary cases; the Goblin-hours prompt addendum tests were removed with the addendum (ADR-002 §Addendum 2026-05-17). _(`TemplateLabelerTest` exercises all six labels plus precedence (Aftermath > Goblin / Tunnel exit; Decision spiral > Tunnel exit; Concrete shoes > Goblin hours), boundary cases (5am out, case-insensitive tag matching, trimmed energy), and defensive miscasts (non-string tag entries, null-value ambiguous tags). This is **not** representative end-to-end validation on natural transcripts; the structural defect below explains why.)_
