...
Build the end-to-end capture loop: user records or types → foreground call returns transcription + follow-up at human pace → background 3-lens pipeline runs the multi-lens extraction → convergence resolver writes consensus/candidate/ambiguous fields → entry persists to ObjectBox (per ADR-017; markdown is generated at export only). By the end of Phase 2, voice-in produces saved entries with the full content schema populated, and three of the five stop-and-test points (STT-B, STT-C, STT-D) are resolved.
...
- [x] Agent-emitted template labels work for all six archetypes on representative sample transcripts. _(**Won't pass as designed — structural defect found 2026-05-17.** `TemplateLabeler` consumes only CONSENSUS fields; archetype triggers are inferences only the Inferential lens emits → resolves CANDIDATE → discarded → every realistic entry is `AUDIT`. Non-AUDIT only fires when the user speaks the exact internal trigger vocabulary, which is the keyword-stuffed fixture, not a real test. Code stays in place through Phase 2/3; the UI yank is queued in Phase 4 §Story 4.16 and has not landed on this branch yet. Root cause + redesign condition: `backlog.md` §`archetype-template-labeling`. Story 2.10 below carries the full trace.)_ **Superseded 2026-05-24:** now passes — `template_label` is model-emitted + convergence-voted (the model commits; `TemplateLabeler` is reduced to a goblin-window clock predicate). Six distinct archetypes on-device, STT-H 2026-05-24. See `backlog.md` §`archetype-template-labeling` (RESOLVED) + ADR-002 §Addendum (2026-05-24).
- [x] Background extraction populates the canonical schema on the reference device. **Latency target is ADR-002 sequential for v1**. STT-F is the current runtime baseline: ~14.7 s per uninterrupted lens, ~44 s per 3-lens entry on E4B GPU, inside the 25–55 s entry band. Story 2.7's older GPU re-run remains historical evidence, not the current per-lens number. The 30–90s ceiling remains the timeout guard.
...
**Done when:**
- [x] `:core-inference` exposes `CaptureSession` with explicit states: `IDLE`, `RECORDING`, `INFERRING`, `TRANSCRIBED`, `RESPONDED`, `ERROR`. Transitions are explicit; illegal transitions throw. _(Superseded by `CaptureViewModel.CaptureUiState`; voice foreground now persists transcription + inline follow-up from the terminal foreground result.)_
...
**Notes / risks:** The transcript is the entry's text-only history. `entry_text` must be derived from the USER transcription only so retrieval, slugging, and pattern counts stay grounded in the user's words. The original "one entry corresponds to one full session, multiple turns" framing was retired with the STT-B v1 scope choice (see `adrs/ADR-005-stt-b-scope-and-v1-single-turn.md`, which amends `adrs/ADR-002-multi-lens-extraction-pattern.md` §"Multi-turn behavior"). **Superseded 2026-05-17:** `CaptureSession` + `Transcript`/`Turn`/`Speaker` (and `CaptureSessionTest`/`TranscriptTest`) were deleted — proven dead with zero production consumers once Story 2.16 streaming made `CaptureViewModel.CaptureUiState` + the `ForegroundStreamEvent` path the live capture lifecycle and `saveAndExtract`/`EntryStore` the persistence path.

---

### Story 2.2 — Foreground inference: audio → transcription + follow-up

**As** the AI implementor, **I need** the foreground inference call that takes a normalized audio buffer (from Story 1.4) and returns a structured `{transcription, follow_up}` response, **so that** the user gets text back at human conversation pace and the entry transcript renders with its prompt.
...
### Story 2.6.6 — Concurrent lens execution via one Engine, many contexts — **WITHDRAWN 2026-05-17 (SDK-impossible on 0.11.0 — STT-F; v1 ships sequential)**

**Corrected 2026-05-16, then disproven on-device 2026-05-17.** The 2026-05-11 probe was mis-scoped (searched for `Session.clone()`). The 2026-05-16 AAR bytecode probe found `Engine.createSession` / `Engine.createConversation` exist and inferred "one Engine → many independent contexts." **That inference was wrong.** Method presence is not runtime permission. On-device (`RamWallClockProbeTest`, S24 Ultra, 2026-05-17 — see [`stt-results/stt-f-2026-05-17.md`](../stt-results/stt-f-2026-05-17.md)) the pinned `litertlm-android:0.11.0` rejects a second concurrent conversation with `FAILED_PRECONDITION: A session already exists. Only one session is supported at a time.` One Engine = **one live session, serialized**.

This story's concurrent-multi-context mechanism is **withdrawn** — not deferred, not backlog: the SDK forbids it on 0.11.0. ADR-008 §Correction (2026-05-16) is superseded by [ADR-008 §Addendum (2026-05-17)](../adrs/ADR-008-parallel-lens-execution.md). v1 ships the **sequential** 3-lens worker on one serialized session. Re-open only if a future SDK lifts single-session (verify on-device, never from bytecode-method-presence).

**Resolution (2026-05-17, measured).** `LiteRtLmEngine` adds a `callMutex` that holds the createConversation→close lifetime exclusively: at most one session is ever live (the `stateMutex`/`closing`/`drainGate` close-drain machinery is unchanged and orthogonal). `BackgroundExtractionWorker` runs lenses sequentially via `LENSES.map { runLens(...) }` (no `coroutineScope`/`async`/`awaitAll`); the accumulator is a plain `mutableListOf` since there are no concurrent writers. The 2-of-3 fallback, per-lens retry, and `RUNNING`-once contract are unchanged. Sequential 3-lens ≈ 3 × single-lens ≈ ~44 s/entry on E4B GPU — within the documented 25–55 s band.

**Done-when status (2026-05-17 — sequential, the only buildable shape on 0.11.0):**
- [x] `BackgroundExtractionWorker` iterates `LENSES` sequentially on one serialized session — concurrent fan-out withdrawn (SDK single-session, STT-F). One Engine per process, loaded once (`AppContainer.backgroundEngine`, unchanged).
- [x] `RUNNING` set once before the first lens; terminal `COMPLETED` / `TIMED_OUT` / `FAILED` after all three return or the budget trips.
- [x] Per-lens failure handling preserved: per-lens retry (`maxAttemptsPerLens`), two failures → that lens contributes "no opinion", the other two still feed the resolver (2-of-3 fallback intact). Resolver sees the same `LensResult` shape.
- [x] Latency logged per entry to `VestigeBackgroundExtraction`.
- [x] **RAM / wall-clock recorded on the reference S24 Ultra** — [`stt-results/stt-f-2026-05-17.md`](../stt-results/stt-f-2026-05-17.md): the concurrent run measured PSS Δ ~413 MB and exposed the single-session constraint. Post-fix sequential re-probe must show 3/3 lenses parsed.
- [x] Unit/integration tests cover happy path, one-lens-fail (2-of-3), all-fail (`FAILED`), timeout (`TIMED_OUT`), engine-error retry accounting, **plus a max-in-flight probe pinning the sequential contract** (`lenses run sequentially, never concurrently — SDK is single-session`: max-in-flight == 1, LENSES order). Story 2.6's parsing/retry/no-opinion tests preserved. `LiteRtLmEngine` before-init contract messages pinned for the stream methods after the `callMutex` refactor.
- [x] `Session`-API constraint hit on the pinned `0.11.0` — `FAILED_PRECONDITION` on the second concurrent conversation. The "stop and write a superseding ADR" branch **was** triggered: ADR-008 §Addendum (2026-05-17) supersedes §Correction (2026-05-16).
...
- [x] **Factor 2 — `consensus_with_conflict` reachable on ≥ 2 entries** (`disagree_fields=[]` AND ≥ 1 Skeptical schema-binding flag). _(Reruns #1 + #2: A4 + B2 each.)_
...
- [x] Per-field output follows the four convergence rules: ≥2 of 3 agree → consensus; only one lens populates with two surviving → candidate; lenses disagree → ambiguous (null + note); Skeptical flags conflict → consensus-with-conflict. _(Refines the ADR-002 rules with one v1 strengthening: a **lone surviving lens** (1-of-3 — i.e. two lenses parse-failed) leaves the entry under-evidenced overall; every field — populated or not — resolves to AMBIGUOUS rather than minting candidates from a single witness. Matches ADR-002 §"Edge case — lens errors mid-call" applied per-field. With 2 or 3 surviving lenses, the rule 2 "single-lens-populates → CANDIDATE" path stands.)_
...
- [x] Edge cases per ADR-002 §"Convergence edge cases" are handled: lens parse failure (treat as no opinion, not as agreement), Skeptical-only flag without value (consensus-with-conflict using consensus value), all three null on a nullable field (consensus-null). _(Worker filters `LensResult.extraction == null` before calling the resolver per `BackgroundExtractionWorker.completeRun`; resolver iterates `Lens.entries` and treats absent lenses as no opinion. `tags` follow the per-tag-count rule (consensus = any tag in ≥2 lenses; ordered by Literal then remaining lenses' insertion).)_
...
**As** the AI implementor, **I need** the agent to emit one of six template labels (Aftermath / Tunnel exit / Stalled / Decision spiral / Goblin hours / Audit) per entry based on the convergence-resolved fields, **so that** Phase 3 pattern detection can group entries by archetype and the History view can show category labels.
...
- [x] A `TemplateLabeler` runs after the convergence resolver (Story 2.8) and selects one of six labels using the detection rules in `concept-locked.md` §"Templates (agent-emitted labels, not user-facing modes)":
  ...
  - Stalled: behavioral surface signals stuck task + resistance markers
  ...
  _(`:core-inference/TemplateLabeler.kt` reads `ResolvedExtraction` + capture `ZonedDateTime` and returns a `TemplateLabel`. The zone rides on the timestamp — the labeler no longer consults `ZoneId.systemDefault()` — so a user crossing zones (or a DST shift) between recording and the background extraction run can't relabel the entry. `BackgroundExtractionWorker.extract(...)` now requires that persisted `capturedAt` value and computes the label after convergence, surfacing it on `BackgroundExtractionResult.Success.templateLabel`. Anchor tokens are taken from `surfaces/state.txt` (`crashed`, `tunnel-exit`, `decision-loop`, `late-night`). Stalled resistance markers (`stuck` / `stalled` / `paralyzed` / `blocked` / `resistance` / `task-paralysis`) are kept narrow on purpose — STT-C tag stability will widen or trim the set with measured evidence. Aftermath requires both `energy_descriptor == "crashed"` (case-insensitive, trimmed) AND `state_shift == true`. Goblin hours requires local hour ∈ 0..4 AND a `late-night` / `overnight` tag — concept-locked's "midnight–5am" reads as 00:00 inclusive, 05:00 exclusive.)_
- [x] Label emission rules are encoded in code (not asked of the model) — `TemplateLabeler` is deterministic given the resolved fields. _(No engine reference in `TemplateLabeler`; `core-inference` Lens / engine types are unimported. Same shape as `DefaultConvergenceResolver` per ADR-002 §"Convergence Resolver Contract" — pure data merge after extraction. Only `CONSENSUS` / `CONSENSUS_WITH_CONFLICT` verdicts drive label selection: `CANDIDATE` values (single-lens witnesses) and `AMBIGUOUS` fields are non-load-bearing per `concept-locked.md` §"Convergence rules", since the template label feeds pattern grouping which the convergence rules explicitly bar candidate evidence from until promoted.)_
- [x] Goblin hours additionally triggers context-aware prompting in the foreground call: shorter follow-up cadence, fewer probes, slightly different tone (per `concept-locked.md` §Templates). _(`ForegroundInference` now accepts a `ZoneId` (default `systemDefault()`) and consults `clock.instant()` pre-engine-call: when the local hour is in 0..4 the system prompt appends the classpath resource `/foreground/goblin-hours-addendum.txt` (file at `core-inference/src/main/resources/foreground/goblin-hours-addendum.txt`) after the output-schema reminder. The addendum tells the model to keep the follow-up short, ask one probe instead of several, stay even-toned, and not name the hour. Detection is clock-only — the foreground call has no access to extraction tags yet — so the addendum and the post-extraction `GOBLIN_HOURS` label can disagree (e.g. 3am crashed entries label as Aftermath but still receive the goblin addendum). Both detections are correct: the addendum tunes tone for the current capture; the label categorizes the entry after extraction.)_ **Superseded 2026-05-17:** the foreground goblin-hours addendum was removed entirely (resource, `ForegroundInference` injection, `isGoblinHours`/`GOBLIN_HOURS_*`, the `zoneId` seam, and its tests) per ADR-002 §Addendum (2026-05-17) — the follow-up no longer receives time-of-day steering. The post-extraction `TemplateLabel.GOBLIN_HOURS` label + `time_of_day_cluster` pattern are unaffected.
- [/] Synthetic coverage only: `TemplateLabelerTest` hand-feeds canonicalized field maps for all six labels, plus precedence and boundary cases, and `ForegroundInferenceTest` covers the Goblin-hours prompt addendum. _(`TemplateLabelerTest` exercises all six labels plus precedence (Aftermath > Goblin / Tunnel exit; Decision spiral > Tunnel exit; Stalled > Goblin hours), boundary cases (5am out, case-insensitive tag matching, trimmed energy), and defensive miscasts (non-string tag entries, null-value ambiguous tags). `ForegroundInferenceTest` adds three Goblin-hours system-prompt cases: outside-window absent, inside-window present (after the schema reminder), and 5am-boundary excluded. This is **not** representative end-to-end validation on natural transcripts; the structural defect below explains why.)_
...
**Structural defect (2026-05-17) — feature planned for v1 UI removal:** The code shipped and `TemplateLabelerTest` passes (it hand-feeds CONSENSUS fields), but the *end-to-end* path is structurally broken. `TemplateLabeler` consumes only CONSENSUS / CONSENSUS_WITH_CONFLICT fields (deliberately — the label was specced to feed pattern grouping). `DefaultConvergenceResolver` mints CONSENSUS only on ≥2-of-3 lens agreement. The archetype triggers (`energy_descriptor=="crashed"`, `state_shift`, tags `tunnel-exit`/`decision-loop`/`stuck`/`late-night`) are all inferences — only the Inferential lens produces them from natural behavioral text; Literal (strict) won't unless the user says the literal word, and Skeptical emits flags not corroborating values. Inferential-alone resolves to CANDIDATE, which the labeler discards → every realistic entry is `AUDIT`. The feature only produces a real label when the user speaks the exact internal vocabulary (Literal then also emits it → 2-lens agreement). That is the keyword-stuffed fixture, not a functional test. Decision: code stays inert through Phase 2/3; the **planned** fix is Phase 4 §Story 4.16 removing the label from UI surfaces, but that cleanup has **not** landed on this branch yet; redesign deferred to v1.5. Full root cause + the why-not-an-easy-fix analysis + unblock condition: `backlog.md` §`archetype-template-labeling`. **Superseded 2026-05-24:** the redesign shipped — `template_label` became model-emitted + convergence-voted (every lens emits the archetype directly → CONSENSUS, not the discarded-Inferential-CANDIDATE failure), the model commits via the reworded output-schema, and the tag-based `TemplateLabeler` is deleted down to an `isGoblinHours(capturedAt)` predicate. The defect is resolved, not deferred. See `backlog.md` §`archetype-template-labeling` (RESOLVED) + ADR-002 §Addendum (2026-05-24).
...
### Story 2.12 — Save flow: ObjectBox after convergence

**As** the AI implementor, **I need** the entry to persist to the ObjectBox `Entry` row after the convergence resolver has populated the canonical fields, **so that** the entry is durably saved before the user leaves the session and export can render markdown from one source.

**Done when:**
- [x] Save fires after Story 2.8 convergence resolution completes — not after the foreground call alone. _(`BackgroundExtractionSaveFlow.saveAndExtract` (in `:app/save/`) awaits `BackgroundExtractionWorker.extract`'s terminal `BackgroundExtractionResult` before calling `EntryStore.completeEntry` / `failEntry`. Foreground text persists earlier via `EntryStore.createPendingEntry` which writes `entry_text`, optional `follow_up`, and `extraction_status=PENDING` so the user's words are durable before extraction finishes.)_
- [x] The `Entry` row in ObjectBox carries: `entry_text` (USER transcription or typed text only), optional `follow_up` (voice foreground follow-up, `null` for typed entries), recorded `persona`, `timestamp`, `template_label` (Story 2.10), `tags`, `energy_descriptor`, `recurrence_link`, `stated_commitment`, `entry_observations`, `confidence` (per-field), and the operational triplet from ADR-001 Q3. _(`EntryEntity` persists `entryText` + optional `followUpText` before background extraction completes, then `EntryStore.completeEntry` fills the resolved fields from `ResolvedExtraction.fields[…]` + the `templateLabel` argument.)_
- [x] No markdown file is written during save. _(`EntryStore` writes ObjectBox only; `EntryMarkdownRenderer` renders markdown during export.)_
...
- [x] A smoke test runs a full session through the pipeline and verifies ObjectBox carries the canonical fields. _(End-to-end coverage splits across `EntryStoreTest` for persistence and `BackgroundExtractionSaveFlowTest` for worker-result-to-store routing.)_

**Notes / risks:** No dual-write invariant exists. Phase 4 export-to-zip renders markdown from ObjectBox.
...
- [x] Observations persist in the `entry_observations` field. _(`EntryStore.completeEntry` serializes the `List<EntryObservation>` via `JSONArray` into `EntryEntity.entryObservationsJson`; export renders it as `entry_observations:` inline JSON per `architecture-brief.md` §"Field placement rules".)_
...
**Done when:**
- [x] `LiteRtLmEngine.streamMessageContents(parts: List<Content>): Flow<String>` is added — the multimodal streaming counterpart to `sendMessageContents`. `streamText(prompt)` already exists and wraps `sendMessageAsync` Flow for text-only calls; `streamMessageContents` applies the same pattern to the `AudioFile + Text` foreground path. The SDK's `sendMessageAsync(contents): Flow<Message>` is the underlying API (per official Android docs `https://ai.google.dev/edge/litert-lm/android`). _(Mirrors `streamText` verbatim — one conversation per call, closed in `finally`, `flowOn(ioDispatcher)`. The 0.11.0 `Conversation.sendMessageAsync(Contents, Map): Flow<Message>` overload (Map defaulted) was bytecode-confirmed via `javap` before coding.)_
- [x] `ForegroundInference.runForegroundCall()` switches from `sendMessageContents()` (blocking) to `streamMessageContents()`. The return type changes from a single `ForegroundResult` to a `Flow<ForegroundToken>` or equivalent streaming shape. _(Current voice path returns `Flow<ForegroundStreamEvent>` with `Transcription` / `FollowUpDelta` / `Terminal`; typed entries persist directly.)_
- [x] `ForegroundResponseParser` is updated to parse XML tags (`<transcription>`, `<follow_up>`) from a streamed token buffer rather than a completed string. The parser must handle tag boundaries arriving mid-token. _(New `ForegroundStreamScanner` owns the incremental surfacing on a cumulative buffer — a tag split across chunks (`</transcr`+`iption>`) resolves once both halves land; a partial `</follow_up` is withheld until disambiguated. The **authoritative** verdict (ordering, duplicate blocks, empty bodies) stays with the existing `ForegroundResponseParser` run on `scanner.accumulated` at stream end — no duplicated parse logic, the terminal parse is the safety net for any held-back tail.)_
- [x] `CaptureViewModel.Reviewing` state carries a `followUpText: String` field that appends tokens as they arrive. The capture screen renders the growing string in real time. _(`ReviewState.followUp` grows via `showReviewing` on each `FollowUpDelta`; `Transcription` surfaces first, `Terminal(Success)` replaces the streamed approximation with the authoritative parsed value + saves. `CaptureScreen` already renders `ReviewState.followUp` verbatim — no UI structural change needed.)_
- [x] Temp WAV file deletion still fires in `finally` after the stream completes or is cancelled — audio discard contract (ADR-001 §Q8) is unchanged. _(`finally { discardTempWav(temp); activeTempWavs -= … }` wraps the collect inside the voice `flow {}`; runs on normal completion, parse failure, engine throw, and collector cancellation alike. Pinned by `ForegroundInferenceTest` cases.)_
- [x] Coroutine cancellation: if the user navigates away during streaming, the Flow is cancelled and the partial result is discarded. No partial entries are saved. _(`saveAndExtract` fires only on `Terminal(Success)`; a collector cancel throws out the partial before any Terminal — `ForegroundInferenceTest."mid-stream cancellation deletes the temp WAV and never emits Terminal"` + `CaptureViewModelTest."streaming deltas grow Reviewing follow-up and only Terminal saves"`.)_
- [x] Existing `ForegroundInferenceTest` suite updated to exercise streaming happy path, mid-stream cancellation, and parse-failure on incomplete XML tag at stream end. _(Whole suite ported to the streaming surface (engine stubbed via `streamMessageContents` Flow; `.terminal()` helper drives to the verdict) preserving every WAV-privacy / sweep / goblin / persona intent per the story's "don't refactor the tests away" note. Added: split-tag, delta-reconstruction, mid-stream-cancel, incomplete-`</follow_up>` cases + a dedicated `ForegroundStreamScannerTest` (pos/neg/edge: split open & close tags, partial-close holdback, once-only transcription, empty body, bounded follow-up).)_
- [ ] On-device: subjective latency improvement is visible on the reference S24 Ultra. Time-to-first-token logged under `VestigeForegroundInference`. _(**Manual-check stop — on-device subjective improvement. Not self-ticked.** The loggable half is shipped: `emitEnvelope` emits `Log.d("VestigeForegroundInference", "<label> persona=… ttft=<ms>ms")` on the first raw model chunk (pre-scan — records when the model emits its first token, not when the scanner closes a tag). The S24 Ultra subjective check awaits user outcome.)_
...
### Story 2.19 — Concurrent inference: eliminate foreground-blocks-on-background mutex contention — **CONCURRENT PATH WITHDRAWN 2026-05-17; CANCELLATION QUEUE LANDED 2026-05-20**
...
**Goal withdrawn 2026-05-17 — not achievable on `litertlm-android:0.11.0`.** The story assumed one Engine could host concurrent independent contexts (foreground + background). On-device measurement (`RamWallClockProbeTest`, S24 Ultra — [`stt-results/stt-f-2026-05-17.md`](../stt-results/stt-f-2026-05-17.md)) proved the SDK enforces **one live session per Engine**: a second concurrent `createConversation` throws `FAILED_PRECONDITION`. There is no Kotlin-layer design that makes foreground non-blocking while a background lens holds the only session — the constraint is in the SDK, not in our locking. Two Engines is out (2× weight load, ~7.3 GB E4B). The non-blocking guarantee is therefore **unachievable in v1** and the story is withdrawn, not deferred. See [ADR-008 §Addendum (2026-05-17)](../adrs/ADR-008-parallel-lens-execution.md) (supersedes §Correction 2026-05-16).

**Historical v1 fallback (2026-05-17).** `LiteRtLmEngine.callMutex` serializes every call (Story 2.6.6 resolution). At that point, a foreground record-tap during an in-flight background lens waited for that lens call to finish (≤ one lens ≈ ~15 s on E4B GPU), then ran. That number came from STT-F's single-lens measurement and is no longer the current foreground wait contract.

**Update 2026-05-20.** Concurrent sessions remain impossible, but foreground no longer waits for background to finish. Foreground cancels active background extraction; background work requeues FIFO and reruns after foreground releases the single session. Background extraction duration remains the measured sequential number: ~14.7 s per uninterrupted lens, ~44 s for three lenses, inside the 25–55 s entry band.
...
**Done when (withdrawn — recorded for the supersede trail):**
- [x] ~~A foreground call that arrives during an active background extraction does not block on the engine Mutex.~~ **Withdrawn — SDK-impossible (STT-F).** Foreground serializes behind the in-flight lens via `callMutex`; this is the only single-session-safe behavior. Accepted for v1.
- [x] ~~Background extraction resumes after the foreground call completes without user action.~~ **Withdrawn** — no preemption exists to resume from; calls serialize.
- [x] All existing `BackgroundExtractionWorkerTest` + `LiteRtLmEngineTest` cases pass, plus the sequential-contract probe. _(JVM suites green locally; full gate on push.)_
- [x] `architecture-brief.md` §"AppContainer Ownership" `InferenceCoordinator` row + ADR-008 record the measured single-session reality (ADR-008 §Addendum 2026-05-17 supersedes §Correction 2026-05-16; STT-F is the evidence).

**Notes / risks:** Do not instantiate two `LiteRtLmEngine` objects — 2× weight load is prohibitive on E4B. One Engine, one serialized session is the only viable shape on `0.11.0`. Concurrency is not a tuning knob here; it is SDK-gated.
...
- [x] All fifteen stories above are Done or have an explicit, recorded fallback. (Story 2.6.6 — concurrent multi-context per ADR-008 — added 2026-05-10. The interim ADR-009 "SDK-impossible" supersede was a mis-scoped-probe mistake, **deleted 2026-05-16**; ADR-008 restored. **2026-05-17: on-device measurement (STT-F, `stt-results/stt-f-2026-05-17.md`) proved `litertlm-android:0.11.0` is single-session — concurrent multi-context is SDK-impossible. Stories 2.6.6 + 2.19 concurrency is WITHDRAWN; v1 ships the ADR-002 sequential 3-lens worker on one serialized session.** ADR-008 §Addendum (2026-05-17) supersedes §Correction (2026-05-16). The original ADR-009 instinct was right (SDK can't do it) even though its 2026-05-11 reasoning was wrong — net: sequential, as ADR-002 always specified.)
...
- [x] Latency budget on the reference device is recorded (foreground per turn, background per entry). _(Foreground per-turn: 24.4 / 31.2 / 33.4 s on E4B CPU post-fallback (Story 2.3 smoke). Current background baseline: STT-F measured ~14.7 s per uninterrupted lens and ~44 s per 3-lens entry on E4B GPU, inside the 25–55 s entry band. Both are outside the ADR-002 §"Latency budget" 1–5 s foreground target — GPU / NPU latency work lives in Phase 4/5.)_
...
- [x] No new entries logged to `backlog.md` from Phase 2 work that change the v1 contract beyond what an STT fallback already required. _(The earlier `parallel-lens-execution-via-clone` backlog entry came from the mistaken ADR-009 supersede; ADR-009 is deleted and that entry is retired. STT-F reversed the concurrent multi-context premise on 2026-05-17; the current contract is single-session sequential extraction with foreground cancellation + FIFO background rerun per ADR-008/014 addendums 2026-05-20.)_
