...
### Story 2.6.6 — Concurrent lens execution via one Engine, many contexts — **IMPLEMENTED 2026-05-17 (operator-adopted; on-device RAM/wall-clock = manual-check stop)**
...
This story is **not** superseded and **not** a `backlog.md` SDK-gap entry. It was scope-deferred pending a measured adoption call; **the operator made that call on 2026-05-17 ("we do not want to ship sequential")**, so the concurrent mechanism is now implemented.

**Resolution (2026-05-17, operator-adopted).** The mechanism is the readers/writer redesign of `LiteRtLmEngine` (ADR-008 §Correction): the exclusive per-call `Mutex` is replaced with a `withEngine` guard — calls are concurrent "readers" each opening an **independent** SDK conversation off the one Engine; `close()` is the exclusive "writer" that drains in-flight calls before freeing the native handle. `BackgroundExtractionWorker` now fans the three lenses out via `coroutineScope { … async … }.awaitAll()` instead of the sequential `for (lens in LENSES)` loop; `runLens` was made pure (no shared `RunState`, no per-lens listener) so the three concurrent lenses can't race, and `modelCallCount` / `lastError` are derived from the returned `LensResult`s. A thread-safe `CopyOnWriteArrayList` accumulator preserves the partial-results-on-timeout contract under structured-concurrency cancellation. `RUNNING` is emitted once at fan-out (per the reframed bullet); the per-lens-retry `RUNNING` was removed. Per ADR-008 §Correction the single GPU still serializes at its command queue — this is non-blocking structure, **not** a literal 3× speedup.

**Done-when status (2026-05-17):**
- [x] `BackgroundExtractionWorker` no longer iterates `LENSES` sequentially — concurrent fan-out via `coroutineScope`/`awaitAll` over independent contexts (no `Session.clone()`, no CoW shared prefix — corrected mechanism). One Engine per process, loaded once (`AppContainer.backgroundEngine`, unchanged).
- [x] `RUNNING` set once at fan-out; terminal `COMPLETED` / `TIMED_OUT` / `FAILED` after all three return or the budget trips.
...
- [ ] **RAM budget verified on the reference S24 Ultra** (Engine 3.66 GB + N concurrent KV contexts under the runtime ceiling). _(**Manual-check stop — on-device. Not self-ticked.** This is the measurement ADR-008 §Correction names as the adoption input; the operator adopted concurrency by directive, but the RAM/wall-clock figure is still unrecorded and awaits an S24 Ultra run.)_
- [x] Unit/integration tests cover happy path, one-lens-fail (2-of-3), all-fail (`FAILED`), timeout (`TIMED_OUT`), engine-error retry accounting, **plus a new max-in-flight probe proving the three lenses are genuinely concurrent** (`the three lenses run concurrently, not serialized`). Story 2.6's parsing/retry/no-opinion tests were preserved, not refactored away (listener expectations updated to single-RUNNING per the reframed bullet). `LiteRtLmEngine` before-init contract messages pinned for the stream methods after the `withEngine` refactor.
- [x] No `Session`-API error on the pinned `0.11.0` (`createConversation` is the proven path already in shipping code) — the "stop and write a superseding ADR" branch was not triggered.
...
- [ ] On-device: subjective latency improvement is visible on the reference S24 Ultra. Time-to-first-token logged under `VestigeForegroundInference`. _(**Manual-check stop — on-device subjective improvement. Not self-ticked.** The loggable half is shipped: `emitEnvelope` emits `Log.d("VestigeForegroundInference", "<label> persona=… ttft=<ms>ms")` on the first surfaced event. The S24 Ultra subjective check awaits user outcome.)_
...
**Done when:**
- [x] After voice transcription returns (or typed text is submitted), `CaptureViewModel` calls `RetrievalRepo.query(transcriptionText, topN = 3)` before generating the follow-up. _(**Option C** per the operator call + `architecture-brief.md` §"Retrieval History Gap": voice is a two-call flow — call 1 streams the transcription from the audio (`runForegroundCall`, cancelled at the `Transcription` event so no follow-up is wastefully generated and the temp WAV is discarded by the existing `finally`); the transcription then feeds `retrieveHistory(query)`; call 2 (`runForegroundTextCall`) streams the history-conditioned follow-up. Typed is single-call: `query(typed text)` upfront → `runForegroundTextCall(text, persona, history)`. `AppContainer.retrieveHistory` runs `RetrievalRepo.query(query, topN = 3)`.)_
- [x] The retrieved history is threaded into `ForegroundInference.runForegroundCall()` (or `runForegroundTextCall()`). `PersonaPromptComposer` or the prompt construction must incorporate the history chunks — review how `PromptComposer` currently handles `retrievedHistory` for background calls and apply the same shape to the foreground prompt. _(`ForegroundInference.runForegroundTextCall(text, persona, retrievedHistory)` renders a `## PRIOR ENTRIES (context only …)` block after the output-schema reminder via `renderForegroundHistory`, mirroring `PromptComposer`'s background budget (cap 3 chunks, 600 chars each, same ellipsis). Voice call 1 stays the proven combined audio prompt — zero prompt-engineering risk on the transcription leg; the call-1 transcription is authoritative over call 2's echo.)_
- [x] `AppContainer.runForegroundCall()` / `runForegroundTextCall()` signature updated to accept `retrievedHistory: List<HistoryChunk>` if not already wired through. _(`AppContainer.runForegroundTextCall(text, persona, retrievedHistory = emptyList())` + new `AppContainer.retrieveHistory(query)`; `saveAndExtract` already accepted `retrievedHistory` and now receives it from the capture path so background extraction is context-aware too. `runForegroundCall` (voice call 1) is unchanged — it has no history by design. `CaptureViewModel` gets a `HistoryRetrieval` seam; `SaveAndExtract` / `ForegroundTextInferenceCall` gained the history parameter; `MainActivity` wires `container::retrieveHistory`.)_
...
- [x] If `RetrievalRepo` returns empty (no prior entries, or embeddings not yet backfilled), the foreground call proceeds normally with no history block — degraded retrieval must not prevent the call. _(Two layers: `AppContainer.retrieveHistory` try/catches (rethrowing `CancellationException`) → `emptyList()`; `CaptureViewModel.retrieveHistorySafely` does the same at the seam. Empty history skips the prompt block entirely (`renderForegroundHistory` is only called when non-empty). Logs the exception class only — never the query text, per AGENTS.md §7.)_
- [x] Unit test: `CaptureViewModelTest` verifies that `RetrievalRepo.query()` is called with the transcription text before `runForegroundCall()` fires, and that the returned history is passed through. _(`voice path queries retrieval with the call-1 transcription and threads history to save`, `typed path queries retrieval with the typed text and threads history to save`, `lookup failure degrades to empty history and the capture still completes`, `voice path keeps call-1 transcription authoritative over call-2 echo`; plus `ForegroundInferenceTest` history-block present/absent cases. All affected suites green locally; full gate on push.)_

**Notes / risks:** This is a correctness fix for the gap documented in `architecture-brief.md` §"Retrieval History Gap (Addendum 2026-05-16)". The retrieval call adds ~880ms (embedding cost) + ObjectBox scan to the foreground path latency. With streaming output (Story 2.16), this is masked — the model starts generating before the user would perceive the retrieval delay. Option C makes the voice path two model calls; this is the operator-accepted cost of a history-aware voice follow-up (the call-1 transcription streams immediately, masking call-1 latency). On-device wall-clock of the two-call voice path is a manual-check measurement, not yet recorded.
