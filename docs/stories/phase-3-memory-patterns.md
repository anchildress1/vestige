...
- [x] `VectorBackfillWorker` calls `embedder(buildEmbeddingText(entry))` instead of `embedder(entry.entryText)`. Legacy non-`COMPLETED` rows with stale vectors are cleared and schema-stamped; rows without any vector are left alone and still embed normally once extraction completes.
...
- [x] `AppContainer.launchVectorBackfillIfReady()` triggers the sweep on cold start. Processes paged batches so the worker does not materialize the whole stale set up-front.
...
- [x] **STT-E re-run required.** Original STT-E passed 2026-05-12 but was run with the embedding source bug present (`entry.entryText`, not distilled fields). The pass verdict justifies shipping EmbeddingGemma, but the benchmark numbers are stale. Re-run the STT-E corpus (same 4 cohort queries, same 18-entry set) against the corrected vectors after Story 3.11 completes. Record updated hybrid vs tag-only numbers in ADR-001 §"Addendum (2026-05-12)". (Cleared 2026-05-19: 3/4 hybrid wins with harness seeding `buildEmbeddingText(entry)`.)
