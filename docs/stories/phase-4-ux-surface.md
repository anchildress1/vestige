...
- [x] `ux-copy.md` same-commit: §"Pattern List", §"Pattern Detail", §"Entry detail" lose the `{Aftermath / Tunnel exit / Concrete shoes / Decision spiral / Goblin hours / Audit}` label lines; replaced with the timestamp / pattern-name / pattern-kind rule. `README.md` §About loses the template parenthetical.
