...
**References:** `PRD.md` §Phase 4, `concept-locked.md`, `adrs/ADR-011-design-language-scoreboard-pivot.md` (visual language — supersedes the Mist sections of `design-guidelines.md`; see Addendum 2026-05-14 for the AppTop pill narrowing), `design-guidelines.md` (product framing + forbidden vocabulary + screen list — visual sections superseded by ADR-011), `ux-copy.md` (entire — status-row label now `GEMMA 4 · LOCAL ONLY` / `GEMMA 4 · LISTENING LIVE` per ADR-011 Addendum 2026-05-14, rest holds), `poc/Energy Direction.html` + `poc/energy-tokens.jsx` + `poc/energy-screens.jsx` (canonical visual sources, replace deleted `poc/design-review.md`), `AGENTS.md`, `architecture-brief.md`, `adrs/ADR-002-multi-lens-extraction-pattern.md` §Q3 (re-eval cost story), `adrs/ADR-004-app-backgrounding-and-model-handle-lifecycle.md` (entire — notification permission, tap target, fallback evaluation)
...
- [ ] Capture screen polished per `poc/Energy Direction.html` capture frames + `poc/screenshots/{capture-still,capture-running}.png` — `AppTop` shell with `GEMMA 4 · LOCAL ONLY` ↔ `GEMMA 4 · LISTENING LIVE` swap (pill stays lime in both states per ADR-011 Addendum 2026-05-14; coral is REC button + destructive only), big REC record button (idle: outline; recording: coral fill + pulsing `StatusDot` + live timer + `TickRule` 30s countdown), `sbBars` audio meter primitive while recording, transcription appearing post-inference (latency budget per `adrs/ADR-002-multi-lens-extraction-pattern.md` §"Latency budget" — 1–5 s target unmet on E4B CPU, currently ~24–33 s per `docs/stories/phase-2-core-loop.md` §Story 2.3 device record), entry transcript with muted user transcription + primary model follow-up (single-turn-per-capture per the STT-B v1 scope choice; see `adrs/ADR-005-stt-b-scope-and-v1-single-turn.md`). The Mist `MistHero` / `AudioMeter` halo composition is **not** built — superseded by ADR-011.
...
- [x] `MistHero` primitive per `poc/design-review.md` §3.3 + `design-guidelines.md` §"Component Conventions / MistHero":
  ...
  - Approaching 30s chunk boundary (~25s): thin progress arc on the ring, no copy. _(Scoreboard: `ChunkProgressBar` linear progress + the 28s pre-warn audio cue per the 2026-05-14 ADR-011 Addendum.)_
...
- [x] Persona switcher chrome: top-right pill `WITNESS ▾` (or active persona) per `ux-copy.md` §"Capture Screen / Status row". Tapping opens the persona selector. Per-capture selection (Story 4.12) hooks into this — the chosen persona applies to the next capture's foreground call. _(AppTop right-slot renders the persona pill in Idle (`{persona} ▾`). Hidden during Recording per the 2026-05-14 ADR-011 Addendum — persona is fixed for the duration of a take. The tap-to-open selector sheet is the Story 4.12 hook; 4.5 ships the chrome.)_
