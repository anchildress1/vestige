...
- [x] Inference placeholder lifecycle (absorbed from Story 2.11):
  - Placeholder appears on foreground-call start; holds **≥ 200 ms** to avoid instant-replace flash; otherwise tracks call duration. _(InferringPane renders `READING_PLACEHOLDER` immediately on `CaptureUiState.Inferring` and stays until `Success` / `ParseFailure` resolves — measured ~24-33 s on E4B CPU per Story 2.3, well past the 200 ms minimum.)_
...
- [ ] **Duplicate / redundant content elimination** (per screen-by-screen audit of `Downloads/Photos-3-001/Screenshot_20260516_22*.png`):
  ...
  - **Pattern Detail header card duplicates the Patterns list card** (`Screenshot_20260516_220859_Vestige.png` vs `Screenshot_20260516_221033_Vestige.png`): tapping a pattern card opens a detail screen whose top card IS the card you just tapped, re-rendered at the same size with the same `Audit Template / audit / 3 Audit entries logged. Worth noting. / 3 of 3 entries · Last seen May 16` composition. The user navigates and sees… the same card. **Fix:** Pattern Detail header is replaced by the Pattern Detail-specific composition called out in the bullet above (display title from template label + state-shift line + `StatRibbon` + `TraceBarE`). The list-card composition is **only** rendered on the list — Pattern Detail never renders it.
...
- [ ] **Pattern surface vs Entry surface visual disambiguation** (`Screenshot_20260516_220613_Vestige.png` Entry Detail vs `Screenshot_20260516_220859_Vestige.png` Pattern Detail): currently the two surfaces are visually indistinguishable — same chrome (lone back arrow upper-left, no AppTop on either), same rounded `s1` card composition, same stacked source/transcript rows, same two-pill bottom rail. A user landing on either after one second of glance cannot tell which one they're on. After:
  ...
  - **Entry Detail** display title is the entry's template label (e.g. `AUDIT.`) or date fallback — singular, one moment. Body is one transcript turn + one model turn + (optionally) one observation card. No "Seen in" multi-source section. Bottom rail = `← BACK` + `● NEW ENTRY`.
  - **Pattern Detail** display title is the pattern's name (e.g. `TUESDAY MEETINGS.`) with `EyebrowE` `● ACTIVE · {TEMPLATE_LABEL}` above — explicitly cued as a set, not a singleton. Body is the `StatRibbon` + `TraceBarE` (which Entry Detail never shows) + `Seen in: {N} ENT` source list. Bottom rail = three pills (`Drop` / `Skip` / `Resolved|Restart`), not two.
...
- [ ] **Screen-transition affordances** (loading / sheet / chrome transitions that currently look broken):
  ...
  - **Settings has no back affordance** (`Screenshot_20260516_220500_Vestige.png`): only predictive-back gesture is wired, no `← BACK` chrome. New users won't discover predictive-back as the only exit. After: AppTop's persona slot doubles as the post-onboarding context; bottom rail carries `← BACK` `EyebrowE` link (matches the Entry Detail rail treatment). Same primitive, three call sites.
  ...
  - **Recording screen has ~50% dead vertical space** (`Screenshot_20260516_220422_Vestige.png`): between the chunk progress bar (0s–30s ticks) and the `LEVEL · LIVE` strip there's a vast empty gap. After: `LiveLayout` reorders so `AudioMeter` sits directly under the chunk progress bar (no gap), then `WORD COUNT · EST` card, then dead space (acceptable — the dead space lives below the data, not between two data clusters).
  - **Tag pills on Entry Detail look like action pills** (`Screenshot_20260516_220613_Vestige.png` `battery-yanked` chip vs `Screenshot_20260516_221041_Vestige.png` `Drop` / `Skip` action pills): same `Pill` primitive used for "this content is tagged X" (passive, informational) and "tap me to do X" (action, interactive). Visual semantic conflict — user can't tell tags from actions at glance. After: tags use a distinct `Chip` primitive (smaller, mono `JetBrains Mono`, no border, `dim` ink on `s2` fill) clearly visually subordinate to `Pill` actions. The `Pill` primitive is reserved for interactive affordances only.
