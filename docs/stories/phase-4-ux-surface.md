...
- [ ] Inference placeholder lifecycle (absorbed from Story 2.11):
  ...
  - On `ForegroundResult.ParseFailure` (or a coroutine cancellation / timeout surfaced as an error), the placeholder is replaced by the matching error copy from `ux-copy.md` §"Error States" — not the placeholder text and not a blank space. `ForegroundResult` itself is a sealed `Success | ParseFailure` pair per Story 2.2; cancellation propagates as a thrown `CancellationException` from the coroutine path, not a third sealed variant.
  - Token streaming is **on-deck, not deferred**. Measured non-streaming latency is 25–55 s per entry on GPU and 127–161 s on CPU (Story 2.7); a 30 s wait staring at the placeholder is already a broken-feeling UI before any Phase 5 dry-run can flag it. The only remaining gate is the ADR-002 §Q1 question: does the structured-output parser survive incremental token delivery from LiteRT-LM? That measurement is the unblock — not a hypothetical "if it feels slow" check.
