...
**As** the AI implementor, **I need** every UI surface that renders the agent-emitted template label to stop rendering it, **so that** the demo never shows the structurally-guaranteed `AUDIT` / `Brain Dump` label that makes the product look broken in the 90 s pitch and 5 min walkthrough.
