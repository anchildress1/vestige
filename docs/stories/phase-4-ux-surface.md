...
- [ ] Discard affordance during recording per `adrs/ADR-001-stack-and-build-infra.md` §Q8: secondary `DISCARD · NO SAVE` text link below `STOP · FILE IT`, single-tap, no confirmation, no toast, returns to idle synchronously. `CaptureSession` exposes a `DISCARDED` terminal state reachable only from `RECORDING`. Audio buffer destroyed in the same call stack — no staging escape, no Gemma 4 call, no `Entry` row, no `Undo`. Hidden once foreground call is in flight. Copy: `ux-copy.md` §"Capture Screen — Discard". Visual register: `design-guidelines.md` §"Capture Screen / Discard".
